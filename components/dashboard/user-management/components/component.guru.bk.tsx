...
import { DashboardContext } from "@context/dashboard.context"; import { toTitleCase } from "@lib/slugh";
import React, { useContext, useState } from "react";
import { BsFillEmojiExpressionlessFill, BsPlus } from "react-icons/bs";
import { FaTriangleExclamation, FaUsers } from "react-icons/fa6";
import Skeleton from "react-loading-skeleton";
import FormGuruBk from "./form.guru.bk";
import axios from "axios";
import { toast } from "react-toastify";

const GuruBk: React.FC = () => {
    ...
    const { data, isLoading, mutate } = useSwrAxios('/api/protected/user-management/guidence-conselor', query);

    const onModalOpen = async () => {

        refModal?.current?.options({
            cardclassName: "w-[35rem] max-md:w-full bg-white",
            title: "Tambah Guru BK",
            content: <FormGuruBk toogleClose={refModal.current.close} mutate={mutate} />
        });
    ...
    const onDelete = async(id: number) => {
        ...
        const handleRemove = axios.delete(`/api/protected/user-management/guidence-conselor`, { data: [{ id }], timeout: 300000}).then((res) => res.data);
        await toast.promise(
            handleRemove,
             {
            ...
            content: (
                <>
                    <div className={"flex items-center space-x-3 w-full"}>
                        ...
                        <h1 className="text-gray-500"> Apa ada yakin ingin Ingin Menghapus Guru Bk ?</h1>
                    </div>
    ...
    return (
        <React.Fragment>
            <div className="flex justify-between items-center w-full">
                <h1 className="text-lg text-gray-700 my-5 font-semibold">Data Guru BK</h1>
                ...
                <div className="flex items-center space-x-3">
                    ...
                    <button
                        onClick={onModalOpen}
                        className="cursor-pointer w-full flex items-center justify-center text-white space-x-2 px-3 py-3 rounded-lg bg-orange-500">
                        <span><BsPlus size={22} className="fill-white" /></span>
                        <span className="font-semibold">Tambah Guru BK</span>
                    </button>
                </div>
            </div>
            <div className={`grid ${data?.data?.items?.length < 1 ? "grid-cols-1" : "grid-cols-3"} gap-4 mt-4`}>
                {
                    data?.data?.items?.length < 1 && (
                        <div>
                            <div className="flex justify-center mb-3">
                                <BsFillEmojiExpressionlessFill className="fill-gray-400/50 text-center" size={45} />
                            </div>
                    ...
                    data?.data?.items?.map((val: any, idx: number) => (
                        <Card key={idx} className="bg-gray-50 shadow min-h-[12rem]">
                            <div className="px-3 py-3">
                                <div className="flex items-center space-x-3">
                                    <div className="w-12 h-12 flex justify-center bg-orange-500/50 items-center rounded-full">
                                ...
                                <ul className="my-2 list-none space-y-2 text-sm">
                                    <li>Sepesialisasi : {val?.specialization}</li>
                                    <li>Kelas Binaan : {val?.class_rooms}</li>
                                    <li>Sertifikasi : {val?.sertification}</li>
                                </ul>
                                <div className="grid gap-2 grid-cols-2 my-3">
                                    <button
                                        onClick={() => onModalEdit(val)}
                                        className="w-full p-2 rounded-md text-center cursor-pointer bg-orange-500/80 hover:bg-orange-600/80 text-white">
                                        Edit
                                    </button>
                                    <button
